...
public class Util {
    ...
    public static final int CONTACTS_CODE = 500;

    public static final String MESSAGE_NOTIFICATION = "PostNotification";

    public static String[] cameraPermissions;
    public static String[] storagePermission;

    public static final String FILE_PATH = "https://firebasestorage.googleapis.com/v0/b/cheatapp-27b2a.appspot.com/o/Image%20Files%2Ffile.png?alt=media&token=8d8c3ef8-614e-4fee-9328-835b43c37f99";

    public static boolean containUser(List<Contacts> contactsList, final String userId){
    ...
    public static String getName(List<Contacts> userPhoneList, final String id){
        for(Contacts contact : userPhoneList){
            if(contact.getUid().equals(id)){
    ...
    public static String getUserName(List<User> userPhoneList, final String phone){
        for(User user : userPhoneList){
    ...
    public static String getCountryISO(){
        ...
        return Util.getPhone(iso);
    }

    public static boolean permissionsStorageCheck(Context context){
        boolean result = ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                == PackageManager.PERMISSION_GRANTED;
        return result;
    }

    public static void permissionsStorageRequest(Activity context){
        ActivityCompat.requestPermissions(context, storagePermission, STORAGE_REQUEST_CODE);
    }

    public static boolean permissionsCameraCheck(Context context){
        boolean result = ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                == PackageManager.PERMISSION_GRANTED;

        boolean result2 = ContextCompat.checkSelfPermission(context, Manifest.permission.CAMERA)
                == PackageManager.PERMISSION_GRANTED;

        return result && result2;
    }

    public static void permissionsCameraRequest(Activity context){
        ActivityCompat.requestPermissions(context, cameraPermissions, CAMERA_REQUEST_CODE);
    }

    public static void permissionsContacts(Activity context){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
            ActivityCompat.requestPermissions(context, new String[]{Manifest.permission.WRITE_CONTACTS, Manifest.permission.READ_CONTACTS}, CONTACTS_CODE);
        }
    }

    public static boolean permissionsContactsCheck(Context context){
